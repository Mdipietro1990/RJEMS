...
df = pd.DataFrame([extract(obj) for obj in open("/Users/matt/Downloads/yelp_dataset/yelp_academic_dataset_business.json")])
...
final_df[['latitude', 'longitude']] = (
    final_df[['latitude', 'longitude']].sub(final_df[['latitude', 'longitude']].mean())).divide(
    final_df[['latitude', 'longitude']].std())
...
def cluster_label(lc, t15_counts):
    ...
    for key, value in lc.items():
        temp = float(value) / float(t15_counts[key - 1])
        if temp >= maxi:
            maxi = temp
            index = key - 1
    pdb.set_trace()
